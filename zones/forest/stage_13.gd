...
#	yield( msg, "message_finished" )
	...
	msg = game.show_message( "you are in terrible danger", \
			$cutscenes/introduction_path/path_follow/forest_spirit, Vector2.ZERO, \
			5, false, false )
	yield( msg, "message_finished" )
...
func _on_rage_coin_rage_coin_captured():
	if gamestate.add_event( "rage coin tutorial" ):
		$player.anim_nxt = "idle"
		$player.vel.x = 0
		$player.set_cutscene()
		var msg = game.show_message( "These coins seem to...", \
			$player, Vector2.ZERO, 5, true, false )
		yield( msg, "message_finished" )

		msg = game.show_message( "... increase my attack range", \
			$player, Vector2.ZERO, 5, true, false )
		yield( msg, "message_finished" )
